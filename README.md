...
| 国际化 | `I18nUtil` / `LocaleUtil` / `I18nFormatUtil` / `CollationUtil` / `AcceptLanguageUtil` | 资源包 `i18n/messages*`、`lang` / `Accept-Language`、数字日期与排序 |
...
| 格式 / 排序 | `I18nFormatUtil` / `CollationUtil` | JDK `NumberFormat` / `Collator` | 货币、百分比、瑞典语 `ö` 在 `z` 后 |
| 控制台 | Vue `useI18n` | 无额外前端依赖 | 侧栏中英切换，请求带 `Accept-Language` |
