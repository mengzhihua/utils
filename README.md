...
| 分组 | 工具 | 说明 |
| --- | --- | --- |
| JSON / 配置 | 扁平化、TypeScript 推断、JSON Pointer、`.env` | 不走后端，样例可在 `/#/c/json-ts-local` 自测 |
| 密码学 | NanoID、TOTP、AES-GCM、SHA-1/384/512 | Web Crypto + PBKDF2，密钥不上传 |
| 页面 / 资源 | UTM、MIME、浏览器信息、色板、CSS px/rem | `navigator` / `Intl` / 颜色预览 |
| 文本 / 日历 | 零宽字符、换行符、Unicode 正规化、Markdown TOC、占位文、vCard、ICS | 纯本地生成与清洗 |
