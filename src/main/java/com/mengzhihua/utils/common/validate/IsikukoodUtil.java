...
/**
 * Estonian isikukood (11 digits). Primary weights {@code 1..9,1};
 * fallback {@code 3,4,5,6,7,8,9,1,2,3}. Sample {@code 37601010003} (1976-01-01, male).
 */
public final class IsikukoodUtil {

    private static final int[] PRIMARY = {1, 2, 3, 4, 5, 6, 7, 8, 9, 1};
    private static final int[] SECONDARY = {3, 4, 5, 6, 7, 8, 9, 1, 2, 3};

    private IsikukoodUtil() {
    }
    ...
    public static char checkDigit(String body10) {
        ...
        if (!digits.matches("\\d{10}")) {
            throw new IllegalArgumentException("isikukood body must be 10 digits");
        }
    ...
    public static boolean female(String value) {
        String digits = normalize(value);
        if (digits.length() != 11) {
            throw new IllegalArgumentException("isikukood must be 11 digits");
        }
