...
set "VERSION=%~1"
if "%VERSION%"=="" (
  for /f "usebackq delims=" %%i in (`python -c "import re;from pathlib import Path;t=re.sub(r'<parent>.*?</parent>','',Path('pom.xml').read_text(),flags=re.S);m=re.search(r'<version>([^<]+)</version>',t);print(m.group(1) if m else '1.1.0')"`) do set "VERSION=%%i"
)
